...
class OtpVerificationPage extends StatefulWidget {
  final String phone;
  OtpVerificationPage({Key key, this.phone}) : super(key: key);

  @override
  _LoginPageState createState() => _LoginPageState();
}

class _LoginPageState extends State<OtpVerificationPage> {
  ...
  userLogin() async {
    if(otp != null){
      var payload = {
        "mobile": this.widget.phone,"otp": otp
      };

      Result response  = await authService.verityOTP(payload);
      if(response is SuccessState) {
        ...
        if(response.msg.length > 0) {
          for(int i = 0; i <= (response.msg.length - 1); i++) {
            if(response.msg[i]['param'] == 'mobile' ) {
              setState(() {
                phoneError = response.msg[i]['msg'];
              });
            }
            if(response.msg[i]['param'] == 'password' ) {
              setState(() {
                otpError = response.msg[i]['msg'];
              });
            }
          }
        }
